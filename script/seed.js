...
async function seed() {
  ...
  const codingChallenges = await Promise.all([
    CodingChallenge.create({
      level: 1,
      prompt:
        "useMapToUpperCase - use the Array.protoype.map within the useMapToUpperCase function",
      description:
        "Remember, the map method can only be called on arrays (so you may need to convert the argument to an array). Map each element so the strings are all upper case.",
      testcode:
    ...
    CodingChallenge.create({
      level: 1,
      prompt:
        'use the Array.prototype.filter method to determine if the string has an "@"',
      description:
        'Array.prototype.includes will come in handy to determine if string has an "@"',
      testcode:
        "const useFilter = arr => {return arr.filter(correctEmail => correctEmail.includes('@'));}",
    }),
    CodingChallenge.create({
      level: 2,
      prompt:
      ...
      description:
        "The keys in the string should only be keys attached to the object, not on its internal prototype (aka .proto), to achieve this, use Object.prototype.hasOwnProperty.",
      testcode:
        "function findObjKeys(obj){Object.keys(obj).map(key =>{return `${key}`}).join(',')}",
    }),
    CodingChallenge.create({
      level: 2,
      prompt:
      ...
      description:
        "The rows X columns represents the dimension of a multiplication table. The return value is a multidimensional or one-dimensional array.",
      testcode:
    ...
    CodingChallenge.create({
      ...
      description: "",
      testcode:
        "const alternate = (func) => {let count = 1; return function() {if (count %2 === 1){ count++;return func();} else { count++;}};};",
    }),
    CodingChallenge.create({
      level: 3,
      prompt:
